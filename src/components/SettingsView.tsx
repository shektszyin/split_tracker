...
const SettingsView: React.FC<SettingsViewProps> = ({ 
  ...
  const handleSaveUserEdit = (index: number) => {
      if(tempUserName.trim()) {
          onUpdateUserName(index, tempUserName);
      }
                         ...
                         {editingUserIndex === idx ? (
                             <div className="flex-1 flex items-center gap-2">
                                <input 
                                    ...
                                    className="w-full bg-slate-800 text-white text-base rounded-lg px-3 py-2 border-none focus:ring-1 focus:ring-white"
                                    autoFocus
                                />
                                <button onClick={() => handleSaveUserEdit(idx)} className="p-2 bg-emerald-500/20 rounded-lg text-emerald-400">
