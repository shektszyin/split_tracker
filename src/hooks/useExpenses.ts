...
export const useExpenses = (householdId: string) => {
  ...
  // 1. Live Sync: Fetch and Subscribe
  useEffect(() => {
    const fetchExpenses = async () => {
      ...
      const { data, error } = await supabase
        .from('expenses')
        .select('*')
        .eq('household_id', householdId) // Shared household filter
    ...
    // The Live Subscription: Updates Shek's screen when Yoyo adds an item
  ...
  // 2. Add Expense to Shared Database
  const addExpense = useCallback(async (data: any) => {
    const { error } = await supabase.from('expenses').insert([{
      name: data.name,
      amount: Number(data.amount),
      category: data.category,
      paid_by: data.paidBy, // 'Shek' or 'Yoyo'
